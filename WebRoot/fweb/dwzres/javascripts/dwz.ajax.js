...
function validateCallback(form, callback, confirmMsg) {
	...
	var _submitFn = function(){
		...
		var re = new RegExp("&","g");
		var formData=$form.serialize().replace(re, "#"); 
		var reqData="reqType=ajaxForm"+"&FWebForm="+$form.attr("name")+"&FWebFormData="+formData;
...
function validateTreeCallback(form, callback) {
	var $form = $(form);
	var ids = "";
	var selectedIds="ids";
//	$form.find("input:checked").filter("[name='"+selectedIds+"']").each(function(i){
//		var val = $(this).val();
//		ids += i==0 ? val : ","+val;
//	});
	$form.find(".checked").find(":checkbox").filter("[name='"+selectedIds+"']").each(function(i){
		var val = $(this).val();
		ids += i==0 ? val : ","+val;
	});
	$form.find(".indeterminate").find(":checkbox").filter("[name='"+selectedIds+"']").each(function(i){
		var val = $(this).val();
		ids += ","+val;
	});
	if(!ids){
...
function navTabAjaxDone(json){
	json=FWebMessage.ajaxDone(json);
	DWZ.ajaxDone(json);
...
function navTabSearch(form, navTabId){
	...
	var re = new RegExp("&","g");
	var formData=$form.serialize().replace(re, "#"); 
	var params ="reqType=ajaxForm"+"&FWebForm="+$form.attr("name")+"&FWebFormData="+formData;
...
//	$.pdialog.reload($form.attr('action'), {data: $form.serializeArray()});
	$.pdialog.reload($form.attr('action'), {data: params});
	return false;
}
