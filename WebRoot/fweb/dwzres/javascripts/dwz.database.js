		...
		selectedTodo: function(){
			...
			return this.each(function(){
				...
				$this.click(function(){
					...
					function _doPost(){
						var tableData="ids="+ids;
//						var reqData="reqType=ajaxForm"+"&FWebTable="+selectedIds+"&FWebTableData="+tableData;
						$.ajax({
							type:'POST', url:$this.attr('href'), dataType:'html', cache: false,
							data: function(){
								if (postType == 'map'){
		...
		treeSelectedTodo: function(options){
			
			function _getIds(selectedIds){
				var ids = "";
				navTab.getCurrentPanel().find("div .checked").find(":checkbox").filter("[name='"+selectedIds+"']").each(function(i){
					var val = $(this).val();
					ids += i==0 ? val : ","+val;
				});
				return ids;
